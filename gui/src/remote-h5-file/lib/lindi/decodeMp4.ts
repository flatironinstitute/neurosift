...
export const decodeMp4 = async (chunk: ArrayBuffer, numFrames: number, width: number, height: number) => {
...
const doBatch = async (batch: {start: number, end: number, url: string, width: number, height: number}) => {    
    ...
    const extractFrame = (time: number) => new Promise(resolve => {
