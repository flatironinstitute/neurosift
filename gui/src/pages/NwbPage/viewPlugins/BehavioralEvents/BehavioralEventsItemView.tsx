...
const behavioralEventsReducer = (state: BehavioralEventsData, action: BehavioralEventsAction): BehavioralEventsData => {
...
const BehavioralEventsItemView: FunctionComponent<Props> = ({width, height, path}) => {
    const nwbFile = useNwbFile()
    const [beData, beDataDispatch] = useReducer(behavioralEventsReducer, {seriesNames: [], series: {}})
    const group = useGroup(nwbFile, path)
    useEffect(() => {
        if (!group) return
        let canceled = false
        const load = async () => {
            const timeSeriesGroups = group.subgroups.filter(grp => (grp.attrs['neurodata_type'] === 'TimeSeries'))
...
const BehavioralEventsItemViewChild: FunctionComponent<ChildProps> = ({width, height, beData}) => {
    ...
    const {canvasWidth, canvasHeight, margins} = useTimeScrollView2({width, height: height - timeSelectionBarHeight, hideToolbar, leftMargin})
    ...
    }, [worker, canvasWidth, canvasHeight, margins, visibleStartTimeSec, visibleEndTimeSec])
