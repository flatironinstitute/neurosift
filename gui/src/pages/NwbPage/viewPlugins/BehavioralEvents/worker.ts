...
async function draw() {
    ...
    const coordToPix = (t: number, seriesIndex: number) => {
        ...
        const x = margins.left + (t - visibleStartTimeSec) / (visibleEndTimeSec - visibleStartTimeSec) * (canvasWidth - margins.left - margins.right)
        const y = margins.top + (seriesIndex + 0.5) * (canvasHeight - margins.top - margins.bottom) / (numSeries || 1)
        return {x, y}
    }
