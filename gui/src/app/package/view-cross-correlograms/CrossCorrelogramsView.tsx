...
const CrossCorrelogramsView: FunctionComponent<Props> = ({
  ...
  const listLengthScaler = useMemo(
    () => Math.pow(10, Math.ceil(Math.log10(unitIds.length))),
    [unitIds],
  );
...
const CrossCorrelogramsViewChild: FunctionComponent<ChildProps> = ({
  data,
  width,
  height,
  unitIds,
  ...
  const crossCorrelogramsSorted = useMemo(() => {
    const C = data.crossCorrelograms.filter(
      (a) => unitIds.includes(a.unitId1) && unitIds.includes(a.unitId2),
    );
