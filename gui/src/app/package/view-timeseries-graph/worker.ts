...
const computePlotSeries = (resolvedSeries: ResolvedSeries[]): PlotSeries[] => {
  ...
  resolvedSeries.forEach((rs) => {
    ...
    if ((filteredTimeIndices[0] || 0) > 0) {
      filteredTimeIndices = [
        filteredTimeIndices[0] - 1,
        ...filteredTimeIndices,
      ];
    }
    if (
      (filteredTimeIndices[filteredTimeIndices.length - 1] || tt.length) <
      tt.length - 1
    ) {
      filteredTimeIndices.push(
        filteredTimeIndices[filteredTimeIndices.length - 1] + 1,
      );
    }
    ////////////////////////////////////////////////////////////////////////////////
