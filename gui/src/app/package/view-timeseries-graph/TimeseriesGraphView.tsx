...
const TimeseriesGraphView: FunctionComponent<Props> = ({
  data,
  width,
  height,
}) => {
  ...
  const resolvedSeries = useMemo(
    () =>
      series.map((s) => {
        ...
        if (ds === undefined)
          throw Error(`Dataset not found in series: ${s.dataset}`);
        return {
          ...s,
  ...
  const { visibleStartTimeSec, visibleEndTimeSec } = useTimeRange(
    timeOffset || 0,
  ); // timeOffset is subtracted from start and end after getting from the global state
