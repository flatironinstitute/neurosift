...
const zoomUpdate: DebounceThrottleUpdater<
  ZoomStateProperties,
  ZoomStateRefs
> = (refs, state) => {
  const { divRef } = refs;
...
const zoomResolver: DebounceThrottleResolver<
  ZoomStateRefs,
  ZoomResolverProps
> = (refs, props) => {
...
export const useThrottledZoom = (
  ...
  const resolverProps = useMemo(() => {
    return { zoomTimeseriesSelection };
  }, [zoomTimeseriesSelection]);
  const zoomHandler = useThrottler(
    zoomUpdate,
    zoomResolver,
    refs,
    resolverProps,
