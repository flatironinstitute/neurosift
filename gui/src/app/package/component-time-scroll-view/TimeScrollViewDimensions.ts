...
export const useTimeseriesMargins = (
  timeseriesLayoutOpts: TimeseriesLayoutOpts | undefined,
  manualMargins?: PartialMargins | undefined,
): Margins => {
  return useMemo(() => {
...
export const useFocusTimeInPixels = (timeToPixelMatrix: Matrix) => {
  const { currentTime, currentTimeInterval, currentTimeIsVisible } =
    useTimeseriesSelection();
  const pixelTime = useMemo(() => {
    return {
      currentTimeInPixels:
        currentTime !== undefined && currentTimeIsVisible
          ? convert1dDataSeries([currentTime], timeToPixelMatrix)[0]
          : undefined,
      currentTimeIntervalInPixels:
        currentTimeInterval !== undefined
          ? (convert1dDataSeries(currentTimeInterval, timeToPixelMatrix) as [
              number,
              number,
            ])
          : undefined,
