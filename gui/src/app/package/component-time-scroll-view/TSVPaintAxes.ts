...
export const paintAxes = <T extends { [key: string]: any }>(
...
const paintYTicks = (
  ...
  if (showYMinMaxLabels) {
    ...
    context.fillText(
      stringMax.substring(0, printMax),
      labelRightEdge,
      topMargin,
    );
    context.textBaseline = "top";
    context.fillText(
      datamin.toString().substring(0, printMin),
      labelRightEdge,
      xAxisYCoordinate,
    );
  }
...
const paintPanelHighlights = (
  context: CanvasRenderingContext2D,
  panels: TimeScrollViewPanel<any>[],
  selectedPanelKeys: Set<number | string>,
  topMargin: number,
  width: number,
