import { isArrayOf, isNumber, optional, validateObject } from "@fi-sci/misc";
...
export const isHighlightIntervalSet = (x: any): x is HighlightIntervalSet => {
  ...
  const orderCheck = x.intervalStarts.every(
    (startVal: number, index: number) => startVal <= x.intervalEnds[index],
  );
  const validColorCheck =
    !x.color || (x.color.length > 2 && x.color.length < 5);
