...
import useActionToolbar, {
  OptionalToolbarActions,
} from "./TimeScrollViewActionsToolbar";
import { HighlightIntervalSet } from "./TimeScrollViewData";
import {
  Margins,
  useDefinedMargins,
  useFocusTimeInPixels,
  usePanelDimensions,
} from "./TimeScrollViewDimensions";
import useTimeScrollEventHandlers, {
  suppressWheelScroll,
} from "./TimeScrollViewInteractions/TimeScrollViewEventHandlers";
...
const TimeScrollView = <T extends { [key: string]: any }>(
  props: TimeScrollViewProps<T>,
) => {
  const {
    margins,
    panels,
  ...
  const { panelHeight, panelWidth } = usePanelDimensions(
    width - toolbarWidth,
    height,
    panels.length,
    panelSpacing,
  ...
  const timeToPixelMatrix = use1dScalingMatrix(
    panelWidth,
    visibleStartTimeSec,
    visibleEndTimeSec,
    definedMargins.left,
  );
  const { currentTimeInPixels, currentTimeIntervalInPixels } =
    useFocusTimeInPixels(timeToPixelMatrix);
  ...
  const pixelHighlightSpans = filterAndProjectHighlightSpans(
    highlightSpans,
    visibleStartTimeSec,
    visibleEndTimeSec,
    timeToPixelMatrix,
  ...
  const highlightLayer = useMemo(() => {
    return pixelHighlightSpans.length > 0 ? (
      <TSVHighlightLayer
        width={effectiveWidth}
        height={height}
