...
const TimeAlignedSeriesPlot: FunctionComponent<TimeAlignedSeriesPlotProps> = ({
  width,
  height,
  alignToTimes,
  roiData,
  ...
  const plots: { times: number[]; values: number[]; color: string }[] =
    useMemo(() => {
      ...
      for (let j = 0; j < alignToTimes.length; j++) {
        ...
        while (
          i >= 0 &&
          roiTimestamps[i] > alignToTimes[j] + windowRange.start
        ) {
          i -= 1;
        }
        while (
          i < roiTimestamps.length &&
          roiTimestamps[i] < alignToTimes[j] + windowRange.end
        ) {
...
const PlotChild: FunctionComponent<PlotChildProps> = ({
  ...
  const plotsSampled = useMemo(
    () => samplePlots(plots, prefs.maxNumRois),
    [plots, prefs.maxNumRois],
  );
  useEffect(() => {
