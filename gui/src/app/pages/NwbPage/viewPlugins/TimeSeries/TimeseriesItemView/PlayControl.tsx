...
const PlayControl: FunctionComponent<Props> = () => {
    ...
    return (
        <div>
            {!playing && <IconButton title="Play" disabled={disabled || playing} onClick={handlePlay}><PlayArrow /></IconButton>}
            {playing && <IconButton title="Stop" disabled={disabled || !playing} onClick={handleStop}><Stop /></IconButton>}
            <PlaybackRateControl disabled={playing || disabled} playbackRate={playbackRate} setPlaybackRate={setPlaybackRate} />
        </div>
    )
}
