...
export const selectedItemViewsReducer = (
  state: SelectedItemViewsState,
  action: SelectedItemViewsAction,
): SelectedItemViewsState => {
  switch (action.type) {
    case "toggle":
      if (state.selectedItemViews.includes(action.itemView)) {
        return {
          selectedItemViews: state.selectedItemViews.filter(
            (x) => x !== action.itemView,
          ),
        };
      } else {
...
export const SelectedItemViewsContext = createContext<
  SelectedItemViewsContextType | undefined
>(undefined);

export const useSelectedItemViews = () => {
  const { selectedItemViewsState, selectedItemViewsDispatch } = useContext(
    SelectedItemViewsContext,
  )!;
  const toggleSelectedItemView = useCallback(
    (itemView: string) => {
