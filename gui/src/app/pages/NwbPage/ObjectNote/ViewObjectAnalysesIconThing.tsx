...
const ViewObjectAnalysesIconThing: FunctionComponent<
  ViewObjectAnalysesIconThingProps
> = ({ objectPath, previewText }) => {
  const { contextAnnotations } = useContextAnnotations();
  const theseAnalyses = useMemo(() => {
    if (!contextAnnotations) return undefined;
    return contextAnnotations.filter(
      (a) =>
  ...
  const {
    handleOpen: openAnalyses,
    handleClose: closeAnalyses,
  ...
  return (
    <div>
      <span
        style={{
          color: theseAnalyses && theseAnalyses.length > 0 ? "#242" : "gray",
        }}
      >
        <SmallIconButton
          icon={<Assessment />}
          title={
            !objectPath
              ? "Open analyses"
              : objectPath === "/"
                ? "Open top-level analyses for this file"
                : `Open analyses for ${objectPath}`
          }
          onClick={() => {
            openAnalyses();
          }}
      ...
      {previewText && theseAnalyses && theseAnalyses[0] && (
        <span>&nbsp;&nbsp;{theseAnalyses[0].annotation.text}</span>
      )}
      <ModalWindow visible={analysesVisible} onClose={closeAnalyses}>
