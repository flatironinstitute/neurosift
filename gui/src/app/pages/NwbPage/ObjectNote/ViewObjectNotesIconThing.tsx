...
const ViewObjectNotesIconThing: FunctionComponent<
  ViewObjectNotesIconThingProps
> = ({ objectPath, previewText }) => {
  ...
  return (
    <div>
      <span
        style={{ color: theseNotes && theseNotes.length > 0 ? "#242" : "gray" }}
      >
        <SmallIconButton
          icon={<Note />}
          title={
            !objectPath
              ? "Open notes"
              : objectPath === "/"
                ? "Open top-level notes for this file"
                : `Open notes for ${objectPath}`
          }
          onClick={() => {
            openNotes();
          }}
      ...
      {previewText && theseNotes && theseNotes[0] && (
        <span>&nbsp;&nbsp;{theseNotes[0].annotation.text}</span>
      )}
      <ModalWindow visible={notesVisible} onClose={closeNotes}>
