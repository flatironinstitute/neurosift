...
import { SpecificationsDataset, SpecificationsGroup, SpecificationsNamespace, useNwbFileSpecifications } from "./SetupNwbFileSpecificationsProvider"
...
const SpecificationsView: FunctionComponent<SpecificationsViewProps> = ({ width, height }) => {
    ...
    const {handleOpen: openItem, handleClose: closeItem, visible: itemVisible} = useModalWindow()
    const [selectedItem, setSelectedItem] = useState<SelectedItem | undefined>(undefined)
    const openGroup = useCallback((group: SpecificationsGroup) => {
        setSelectedItem({type: 'group', group})
        openItem()
    }, [setSelectedItem, openItem])
    const openDataset = useCallback((dataset: any) => {
        setSelectedItem({type: 'dataset', dataset})
        openItem()
    }, [setSelectedItem, openItem])
    const openNamespace = useCallback((namespace: SpecificationsNamespace) => {
...
const SelectedItemView: FunctionComponent<SelectedItemViewProps> = ({ selectedItem, closeItem }) => {
    if (!selectedItem) return null
    switch (selectedItem.type) {
        case 'group':
            return <GroupView group={selectedItem.group} closeItem={closeItem} />
        case 'dataset':
            return <DatasetView dataset={selectedItem.dataset} closeItem={closeItem} />
        case 'namespace':
            return <NamespaceView namespace={selectedItem.namespace} closeItem={closeItem} />
    }
}
...
const DatasetView: FunctionComponent<DatasetViewProps> = ({ dataset, closeItem }) => {
    return (
        <div>
...
const NamespaceView: FunctionComponent<NamespaceViewProps> = ({ namespace, closeItem }) => {
    return (
        <div>
