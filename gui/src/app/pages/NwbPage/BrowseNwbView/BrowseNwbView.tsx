...
const BrowseNwbView: FunctionComponent<Props> = ({ width, height }) => {
  ...
  return <MainPanel width={width} height={height} nwbFile={nwbFile} />;
};
  ...
  return (
    <div style={{ position: "absolute", width, height, overflowY: "auto" }}>
      {rootGroup.subgroups.map((sg) => (
        <TopLevelGroupView key={sg.name} nwbFile={nwbFile} name={sg.name} />
      ))}
      <hr />
  ...
  return (
    <div style={{ marginLeft: 10, userSelect: "none" }}>
      ...
      >
        ...
        <GroupTitlePanelText name={name} group={group} nwbFile={nwbFile} />
      </div>
      {expanded && group && (
        <TopLevelGroupContentPanel
          name={name}
