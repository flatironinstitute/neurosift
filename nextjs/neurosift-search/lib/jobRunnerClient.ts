...
export class JobRunnerClient {
  ...
  constructor() {
    ...
    if (!subscribeKey || !publishKey) {
      throw new Error(
        "PubNub configuration missing. Please check your environment variables.",
      );
    }
...
// Global client instance that persists across serverless requests
let globalJobRunnerClient: JobRunnerClient | null = null;

export function getJobRunnerClient(): JobRunnerClient {
  if (!globalJobRunnerClient) {
    globalJobRunnerClient = new JobRunnerClient();
  }
  return globalJobRunnerClient;
}
