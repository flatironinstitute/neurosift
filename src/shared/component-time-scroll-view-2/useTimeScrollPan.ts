...
import { useThrottler } from "./rateLimiters";
...
const setNextPanUpdate: DebounceThrottleUpdater<
  PanUpdateProperties,
  PanUpdateRefs
> = (refs, state) => {
...
const panResolver: DebounceThrottleResolver<PanUpdateRefs, PanResolverProps> = (
  refs,
  props,
) => {
  const { panStateRef } = refs;
  ...
  const deltaPx =
    (panStateRef.current.anchorX ?? 0) - (panStateRef.current.pannedX ?? 0);
  if (deltaPx === 0) return;

  panStateRef.current.anchorX = panStateRef.current.pannedX;
  if (panTimeseriesSelectionDeltaT)
    panTimeseriesSelectionDeltaT(secondsPerPixel * deltaPx);
  // Don't reset panning state by default here--user may still be holding the mouse button
};

export const useThrottledPan = (
  refs: PanUpdateRefs,
  secondsPerPixel: number,
  panTimeseriesSelectionDeltaT: PanFn,
) => {
  const resolverProps = useMemo(() => {
    return { secondsPerPixel, panTimeseriesSelectionDeltaT };
  }, [secondsPerPixel, panTimeseriesSelectionDeltaT]);
  const panHandler = useThrottler(
    setNextPanUpdate,
    panResolver,
    refs,
    resolverProps,
...
const useTimeScrollPan = (
  ...
  const refs = useMemo(() => {
    return { divRef, panStateRef };
  }, [divRef, panStateRef]);
  const { throttler, cancelThrottled } = useThrottledPan(
    refs,
    secondsPerPixel,
  ...
  const startPan = useCallback(
    (mouseX: number) => startPanning(panStateRef, mouseX),
    [panStateRef],
  );
  const clearPan = useCallback(
    () => clearPanState(panStateRef, cancelThrottled),
    [panStateRef, cancelThrottled],
  );
