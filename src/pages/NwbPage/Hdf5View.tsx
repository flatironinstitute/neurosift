...
const TopLevelGroupView: FunctionComponent<TopLevelGroupViewProps> = ({
  ...
  const itemCount = group ? group.subgroups.length + group.datasets.length : 0;
  ...
  return (
    <div className="hdf5-tree-node">
      ...
      >
        ...
        <span
          className={`tree-icon ${expanded ? "icon-folder-open" : "icon-folder"}`}
        >
          {expanded ? <FaFolderOpen /> : <FaFolder />}
        </span>
        <span className="tree-label">{name}</span>
        {itemCount > 0 && <span className="tree-meta">{itemCount} items</span>}
      </div>
      {expanded && group && (
