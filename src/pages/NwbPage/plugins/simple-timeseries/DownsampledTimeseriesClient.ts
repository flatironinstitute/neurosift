...
export class DownsampledChunkedTimeseriesClient {
    ...
    static async create(
        ...
        const { downsampleFactor, downsampleMethod = 'lttb', ...chunkOptions } = options;
        const baseClient = await ChunkedTimeseriesClient.create(nwbUrl, group, chunkOptions);
        return new DownsampledChunkedTimeseriesClient(baseClient, downsampleFactor, downsampleMethod);
    }
    ...
    private downsampleLTTB(
        ...
        for (let channelIndex = 0; channelIndex < data.length; channelIndex++) {
            ...
            const xyData: [number, number][] = timestamps.map((t, i) => [t, channelData[i]]);

            // Apply LTTB downsampling
            const downsampled = LTTB(xyData, targetPoints) as [number, number][];

            // Extract data for this channel
            downsampledData.push(downsampled.map((point: [number, number]) => point[1]));

            // Use timestamps from first channel (all channels should have same timestamps)
            if (channelIndex === 0) {
                downsampledTimestamps = downsampled.map((point: [number, number]) => point[0]);
            }
        }
